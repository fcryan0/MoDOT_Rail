...
modot_all_xings <- 
  bind_rows(modot_trra_grades_aadt, modot_up_grades_aadt) %>% 
  mutate(CrossingID = DOTNumber, MoDOT = 1) %>% 
  janitor::clean_names()
...
gcisCurrent <-
  ...
  left_join(modot_all_xings %>% select(
    ...
    app1_grade = appr_grade_1,
    app1_grade_perc = appr_grade_percent_2,
    app1_dir = appr_dir_3,
    app2_grade = appr_grade_4,
    app2_grade_perc = appr_grade_percent_5,
    app2_dir = appr_dir_6,
    hwy_road_street,
    aadt_modot = aadt,
    aadt_year_modot = aadt_year
    )) %>% 
  mutate(
...
xings_pub_MO_amtrak <- gcisCurrent %>% 
  filter(PosXing == 1, !is.na(Latitude), TypeXing == 3) %>% 
  st_as_sf(coords = c("Longitude", "Latitude"), crs = 4326) %>% 
  st_transform(crs) %>% 
...
xings_priv_MO_amtrak <- gcisCurrent %>% 
  filter(PosXing == 1, !is.na(Latitude), TypeXing == 2) %>% 
  st_as_sf(coords = c("Longitude", "Latitude"), crs = 4326) %>% 
  st_transform(crs) %>% 
  st_filter(rail_amtrak %>% st_buffer(50)) %>% 
  mutate(Pedestrian = if_else(XPurpose == 1, "No", "Yes")) %>% 
